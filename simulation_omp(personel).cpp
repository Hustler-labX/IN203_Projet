...
// make simulation.exe && OMP_NUM_THREADS= ./simulation.exe 
#include <cstdlib>
#include <random>
#include <iostream>
#include <fstream>
...
#pragma omp parallel for schedule(static,contexte.taux_population/num_threads) 
...
int main(int argc, char* argv[])
{
    ...
    for (int i=0; i<argc; i++) {
      ...
      if (std::string("-nw") == argv[i]) affiche = false;
  		...
  		if (affiche) std::cout<<"false"<<std::endl;
  		else std::cout<<"true";
    sdl2::init(argc, argv);
    {
        simulation(affiche);

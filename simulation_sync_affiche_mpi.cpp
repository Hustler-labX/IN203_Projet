...
/**************************************/
 	...
 	if (rank != 0) // le seul pour l'affichage est le processeur 0 
    	...
    	// j'ai utlisé ce taux pour avoir des resultats plus clair.
	...
	while (!quitting)
	{
		...
		// parallarisation synchrone : 
			
		MPI_Send (grille.getStatistiques().data() , largeur_grille*hauteur_grille, 
		mpi_StatistiqueParCase_type , 0, 0, MPI_COMM_WORLD );
